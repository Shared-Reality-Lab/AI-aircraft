...
#         tree = ET.parse('trajectories.xml')
#         root = tree.getroot()
...
#         self.client.sendDREF(dref_user_acc_x, 0.0)
#         self.client.sendDREF(dref_user_acc_y, 0.0)
#         self.client.sendDREF(dref_user_acc_z, 0.0)
...
#         return aircraft_list, xml_list


#     def main_loop(self, aircraft_list, xml_list):
...
#             wp_lat[ac.ID] = xml_list[i][4]
#             wp_lon[ac.ID] = xml_list[i][5]
#             wp_vel[ac.ID] = xml_list[i][6]

#             # Initialize end of movement flag
#             flag_end_movement[ac.ID] = 0

#             flag_speed_null[ac.ID] = 0
...
#             if flag_end_movement == [1]*20 :
#                 break

#             # Loop that goes through every aircrafts (ac == Aircraft object, ac.ID == integer aircraft ID)
#             for ac in aircraft_list :
#                 # User aircraft not included in simulation
#                 if ac.ID != 0 :
#                     # Test if current aircraft needs to be moved
#                     if flag_end_movement[ac.ID] == 0 :

#                         flag_speed_null[ac.ID] = ac.correct_speed(wp_vel[ac.ID], flag_speed_null[ac.ID])
#                         # Stops simulation if an ai aircraft is not moving for 40 ms
#                         if flag_speed_null[ac.ID] == 40 :
#                             print(f'Error : aircraft {ac.ID} not moving')
#                             return
#                         ac.correct_heading(wp_lat[ac.ID], wp_lon[ac.ID])
#                         flag = ac.reached_target(wp_lat[ac.ID], wp_lon[ac.ID])

#                         if flag == 1 :
#                             waypoint_numer = (index_waypoint[ac.ID] //3) + 1
#                             print(f'wp {waypoint_numer} reached : ac{ac.ID}')

#                             index_waypoint[ac.ID] += 3
#                             flag_end, wp_lat[ac.ID], wp_lon[ac.ID], wp_vel[ac.ID] = ac.next_waypoint(xml_list, index_waypoint[ac.ID])
                            
#                             if flag_end == 1 :
#                                 print(f'end of trajectory : ac{ac.ID}')
#                                 ac.set_breaks(1.0)
#                                 flag_end_movement[ac.ID] = 1                            
...
class Simulation:
    ...
    # def Read_XMLC(self):
    #     """Read information from the conflicts.xml file.

    #         return format : [ [ac_ID, lat-wp, lon_wp] , [ac_ID, lat-wp, lon_wp] , ... ]
    #                                     ^                          ^
    #                             Conflict for first ac       Conflict for second ac
        
    #     """
    #     # Load and analyse XML file
    #     tree = ET.parse('conflicts.xml')
    #     root = tree.getroot()

    #     # Create a main list for all conflicts information
    #     conflicts_dico = {}
        
    #     # Loop that goes through every conflicts
    #     for conflict in root.findall('conflict'):
            
    #         # Get aircraft ID
    #         ac_id = int(conflict.get('n'))
            
    #         # Analyze waypoint conflict the aircraft will have to go to
    #         waypoints = conflict.find('waypoints')
            
    #         # Go through every waypoint coordinates
    #         if waypoints is not None:
    #             waypoint = waypoints.findall('waypoint')[0]
    #             lat = float(waypoint.get('lat'))
    #             lon = float(waypoint.get('lon'))

    #         # Add all information about the current conflict list to the main dico
    #         conflicts_dico[ac_id] = (lat,lon)

    #     return conflicts_dico
    ...
    def Read_XMLSimu(self):
        ...
        """
        # Load and analyse XML file
        # tree = ET.parse(r'C:\Users\Corentin\Desktop\ADAIR_SURF_XPlane_SVS\domitille\simulation.xml')
        tree = ET.parse(r'C:\Users\Corentin\Desktop\ADAIR_SURF_XPlane_SVS\McGill\trajTEST.xml')
        root = tree.getroot()
    ...
    def main_loop(self, aircraft_list, xml_list, followings_dico, intersections_dico):
        ...
        # flag_end_movement[2] = 1 # Ac2 leaves later to follow him
        # ac2 = aircraft_list[2]
        # ac2.set_breaks(1.0)

        # flag_end_movement[6] = 1 # Ac6 leaves later
        # ac6 = aircraft_list[3]
        # ac6.set_breaks(1.0)
        ...
        while True :
            ...
            try :
                ...
                #     # if j == 140 :
                #         # flag_end_movement[6] = 0
                #         # ac6.set_breaks(0.0)
                #         # flag_end_movement[2] = 0
                #         # ac2.set_breaks(0.0)
                ...
                #                 target_speed = ac.correct_speed2(conflicts_dico[ac_ID], conflict_offset) # if ac_ID !=2 else wp_vel[ac_ID]
